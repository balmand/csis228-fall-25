...
export class OrderController{
    ...
    list = async (req, res, next) =>{
        try{
            res.json(await this.orderService.listOrders());
        }catch(e){
            next(e);
        }
    }
    ...
    delete = async (req, res, next) =>{
        try{
            ...
            const ok = await this.orderService.deleteOrder(req.params.id);
            if(!ok){
    ...
    getAllWithDetails = async (req, res, next) => {
        try {
            ...
            const data = await this.orderService.getOrderWithDetails();
            if(!data){
                return res.status(404).json({message: 'Not Found'})
            }
            res.status(200).json(data)
